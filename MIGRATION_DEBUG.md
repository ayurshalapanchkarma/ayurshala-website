# Inventory Core Migration Debug Guide

## Instructions to Find the Failing SQL

1. **Copy each section below into Supabase SQL Editor one at a time**
2. **Click "Run"**
3. **If it fails, report the error with the section number**
...
## SECTION 11: Goods Receipt Notes
```sql
...
When a section fails, provide:
1. **Section number**
2. **SQL statement** (first line of the CREATE TABLE)
3. **PostgreSQL error message**

Example:
```
SECTION 2: inventory_products
Error: Could not create table - column "category_id" references non-existent table
```
