# INVENTORY CORE - SQL ERROR DIAGNOSIS

## Instructions

Go to Supabase SQL Editor and copy-paste EACH section below one at a time.
When one fails, report the error.

---

## TEST 1: Base Categories Table (should work)
```sql
...
## TEST 2: Products Table (depends on categories)
```sql
...
```
**Expected**: ✅ Success (categories exist)
...
## TEST 6: Product Suppliers Junction
```sql
...
```
**Expected**: ✅ Success (products + suppliers exist)
...
```
**Expected**: ✅ Success (suppliers exist)

---

## TEST 10: Purchase Order Items
```sql
...
```
**Expected**: ✅ Success (PO + products exist)

---

## TEST 11: Goods Receipt Notes (GRN)
```sql
...
**Example Report**:
```
TEST 2: inventory_products FAILS

SQL:
CREATE TABLE IF NOT EXISTS inventory_products (...)

Error:
ERROR: Could not find table public.inventory_categories in schema cache.

Fix:
Make sure TEST 1 (inventory_categories) completed successfully.
```

Provide the failing test number and the exact PostgreSQL error message.
