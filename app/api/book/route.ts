...
export async function POST(req: NextRequest) {
  ...
  if (action === 'create-order') {
    ...
    const therapyFee      = parseInt(await getSetting('therapy_advance_fee', '1000'))
    const isCod           = payment_method === 'CASH_ON_ARRIVAL'
    const amount          = booking_type === 'consultation' ? consultationFee : therapyFee
