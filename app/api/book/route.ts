...
export async function POST(req: NextRequest) {
  try {
    ...
    const bookingReceivedHtml = BookingConfirmationCash({ patientName: patientData.full_name, bookingId: booking.booking_id, treatment: treatmentList, date: formattedDate, time: preferred_time })
    
    // Send only to patient (no duplicate admin email)
    if (patientData.email) await resend.emails.send({ from, to: patientData.email, subject: `Booking Confirmed — ${booking.booking_id}`, html: bookingReceivedHtml })

    // Send actionable admin email for COD bookings only
    if (isCod) {
      const adminEmailHtml = AdminNewOfflineBooking({ patientName: patientData.full_name, bookingId: booking.booking_id, treatment: treatmentList, date: formattedDate, time: preferred_time })
      await resend.emails.send({ from, to: 'ayurshalapanchkarma@gmail.com', subject: `New Cash-on-Arrival Booking — ${booking.booking_id}`, html: adminEmailHtml })
  ...
  if (action === 'confirm-booking') {
    ...
    if (isCod) {
      ...
      const adminHtml = AdminNewOnlineBooking({ patientName: patient.full_name, bookingId: booking.booking_id, treatment: treatmentList, date: formattedDate, time: booking.preferred_time, amount: amountLabel })
      const patientHtml = BookingConfirmationOnline({ patientName: patient.full_name, bookingId: booking.booking_id, treatment: treatmentList, date: formattedDate, time: booking.preferred_time, amount: amountLabel })
      
      const emails = [resend.emails.send({ from, to: 'ayurshalapanchkarma@gmail.com', subject: `Booking Confirmed — ${booking.booking_id}`, html: adminHtml })]
      if (patient.email) emails.push(resend.emails.send({ from, to: patient.email, subject: `Booking Confirmed — ${booking.booking_id}`, html: patientHtml }))
  ...
  if (action === 'pay-existing') {
    ...
    console.log('[pay-existing] Environment check:', {
      CASHFREE_APP_ID: hasAppId ? 'SET' : 'MISSING',
      CASHFREE_SECRET_KEY: hasSecretKey ? 'SET' : 'MISSING',
    })

    if (!hasAppId || !hasSecretKey) {
      console.error('[pay-existing] Missing Cashfree credentials!')
      return NextResponse.json(
        {
          success: false,
    ...
    console.log('[pay-existing] Initializing Cashfree...')
    let cashfree
    try {
      cashfree = new Cashfree(CFEnvironment.PRODUCTION, process.env.CASHFREE_APP_ID, process.env.CASHFREE_SECRET_KEY)
      console.log('[pay-existing] Cashfree initialized successfully')
    } catch (initError) {
      console.error('[pay-existing] Cashfree initialization error:', initError)
      return NextResponse.json(
        {
          success: false,
    ...
    try {
      ...
      console.log('[pay-existing] Cashfree order created:', {
        orderId: cashfreeOrder?.data?.order_id,
    ...
    if (!cashfreeOrder?.data?.payment_session_id) {
      console.error('[pay-existing] Payment session ID missing in response:', { cashfreeOrder })
      return NextResponse.json(
        {
          success: false,
          error: 'Invalid payment session response from Cashfree',
          received: { orderId: cashfreeOrder?.data?.order_id },
        },
        { status: 500 }
      )
    }

    console.log('[pay-existing] Returning success with payment session')
  ...
  if (action === 'reschedule-booking') {
    ...
    await supabase.from('bookings_new').update({
      ...
      new_date, new_time,
      is_rescheduled: false, rescheduled_at: null,
      status: 'RESCHEDULED',
    ...
    // Email to patient: request received
    if (rPatient?.email) {
      await resend.emails.send({
        from, to: rPatient.email,
        subject: `Reschedule Request Received — ${booking_id}`,
        html: buildPatientRescheduleRequestEmail({ patient: rPatient, booking, treatmentList, oldDate, newDateFmt, new_time }),
      })
    }

    // Email to admin: approval required
    await resend.emails.send({
      from, to: 'ayurshalapanchkarma@gmail.com',
      subject: `Reschedule Approval Required — ${booking_id} — ${rPatient?.full_name}`,
      html: buildRescheduleRequestEmail({ patient: rPatient, booking, treatmentList, oldDate, newDateFmt, new_time, confirmUrl, cancelUrl }),
    })
...
function buildCancellationClinicEmail({ patient, booking, treatmentList, formattedDate, wasOnline }: any) {
  ...
  const logoText = `${baseUrl}/ayurshala_text.png`
  const logoIcon = `${baseUrl}/ayurshala.png`
  const refundAmount = booking.refund_amount > 0 ? booking.refund_amount : booking.amount_paid
  const refundNote = wasOnline
    ? `<div style="margin-top:20px;padding:16px 20px;background:linear-gradient(135deg,rgba(239,68,68,0.08),rgba(239,68,68,0.04));border-radius:12px;border:1px solid rgba(239,68,68,0.25);backdrop-filter:blur(10px)"><p style="margin:0;font-size:13px;color:#dc2626;font-weight:600">⚠️ Refund Required</p><p style="margin:6px 0 0;font-size:12px;color:#7f1d1d">Amount: ₹${refundAmount}\nContact: ${patient?.phone || '—'} · ${patient?.email || '—'}</p></div>`
...
function buildPatientRescheduleRequestEmail({ patient, booking, treatmentList, oldDate, newDateFmt, new_time }: any) {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;background:#FFF3E0;font-family:Arial,sans-serif"><table width="100%" cellpadding="0" cellspacing="0" style="background:#FFF3E0;padding:32px 16px"><tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:24px;overflow:hidden;background:#fff;border:1px solid rgba(232,98,26,0.18);box-shadow:0 8px 40px rgba(232,98,26,0.10)"><tr><td style="background:linear-gradient(135deg,#fff8f0,#ffe8d0);padding:32px 40px;text-align:center;border-bottom:1px solid rgba(232,98,26,0.15)"><h1 style="margin:0;font-family:Georgia,serif;font-size:22px;font-weight:400;color:#1a1008">Reschedule Request Received</h1></td></tr><tr><td style="padding:32px 40px"><p style="font-size:13px;color:#78716c;margin:0 0 20px">Thank you for submitting your reschedule request. Our clinic is reviewing your request and will get back to you shortly.</p><table width="100%" cellpadding="0" cellspacing="0" style="background:#fff8f0;border-radius:14px;border:1px solid rgba(232,98,26,0.12);margin-bottom:20px"><tr><td style="padding:12px 20px;border-bottom:1px solid rgba(232,98,26,0.08)"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#a8a29e">Booking ID</span><br><span style="font-size:15px;color:#E8621A;font-weight:600">${booking.booking_id}</span></td></tr><tr><td style="padding:12px 20px;border-bottom:1px solid rgba(232,98,26,0.08)"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#a8a29e">Treatment</span><br><span style="font-size:15px;color:#E8621A">${treatmentList}</span></td></tr><tr><td style="padding:12px 20px;border-bottom:1px solid rgba(232,98,26,0.08)"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#a8a29e">Current Appointment</span><br><span style="font-size:15px;color:#1a1008;font-weight:600">${oldDate} · ${booking.preferred_time}</span></td></tr><tr><td style="padding:12px 20px"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#a8a29e">Requested New Slot</span><br><span style="font-size:15px;color:#16a34a;font-weight:600">${newDateFmt} · ${new_time}</span></td></tr></table><p style="text-align:center;font-size:12px;color:#c4bdb5;margin:0">We will confirm or contact you within 24 hours. For urgent requests, call +91-9821224767</p></td></tr><tr style="background:#fffaf5"><td style="padding:16px 40px;text-align:center"><p style="margin:0;font-size:11px;color:#c4bdb5">© 2026 Ayurshala Panchakarma Center</p></td></tr></table></td></tr></table></body></html>`
}

function buildRescheduleRequestEmail({ patient, booking, treatmentList, oldDate, newDateFmt, new_time, confirmUrl, cancelUrl }: any) {
  const baseUrl = getBaseUrl()
  const logoText = `${baseUrl}/ayurshala_text.png`
  const logoIcon = `${baseUrl}/ayurshala.png`
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>*{margin:0;padding:0;box-sizing:border-box}</style></head><body style="background:linear-gradient(135deg,#fdf6ee,#ffecd2,#fff8f0);font-family:'Segoe UI',Arial,sans-serif;min-height:100vh;padding:20px"><table width="100%" cellpadding="0" cellspacing="0" style="background:transparent"><tr><td align="center" style="padding:20px 0"><table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:28px;overflow:hidden;background:linear-gradient(135deg,rgba(255,255,255,0.75) 0%,rgba(255,248,240,0.55) 50%,rgba(255,235,210,0.45) 100%);border:1px solid rgba(255,255,255,0.85);box-shadow:0 20px 80px rgba(232,98,26,0.12),0 4px 24px rgba(0,0,0,0.08),inset 0 1px 0 rgba(255,255,255,1);backdrop-filter:blur(40px)"><tr><td style="background:linear-gradient(135deg,rgba(232,98,26,0.08) 0%,rgba(245,166,35,0.06) 100%);padding:40px;text-align:center;border-bottom:1px solid rgba(232,98,26,0.15)"><img src="${logoText}" alt="Ayurshala" width="100" style="height:auto;display:block;margin:0 auto 16px;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.08))"/><h1 style="margin:0;font-family:Georgia,serif;font-size:26px;font-weight:300;color:#1a1008;letter-spacing:0.5px">Reschedule Request Received</h1></td></tr><tr><td style="padding:40px"><div style="background:linear-gradient(135deg,rgba(255,248,240,0.8),rgba(255,240,225,0.6));border-radius:18px;border:1px solid rgba(232,98,26,0.12);padding:24px;backdrop-filter:blur(10px)"><table width="100%" cellpadding="0" cellspacing="0"><tr><td style="padding:12px 16px;border-bottom:1px solid rgba(232,98,26,0.08)"><span style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:#a8a29e;font-weight:600">Patient</span><br><span style="font-size:16px;color:#1a1008;font-weight:600">${patient?.full_name}</span></td></tr><tr><td style="padding:12px 16px;border-bottom:1px solid rgba(232,98,26,0.08)"><span style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:#a8a29e;font-weight:600">Booking ID</span><br><span style="font-size:15px;color:#E8621A;font-weight:600">${booking.booking_id}</span></td></tr><tr><td style="padding:12px 16px;border-bottom:1px solid rgba(232,98,26,0.08)"><span style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:#a8a29e;font-weight:600">Treatment</span><br><span style="font-size:15px;color:#E8621A">${treatmentList}</span></td></tr><tr><td style="padding:12px 16px;border-bottom:1px solid rgba(232,98,26,0.08)"><span style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:#a8a29e;font-weight:600">Old Date</span><br><span style="font-size:15px;color:#78716c;text-decoration:line-through">${oldDate} · ${booking.preferred_time}</span></td></tr><tr><td style="padding:12px 16px"><span style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:#a8a29e;font-weight:600">Requested New Date</span><br><span style="font-size:15px;color:#16a34a;font-weight:600">${newDateFmt} · ${new_time}</span></td></tr></table></div><div style="text-align:center;margin-top:32px;display:flex;gap:12px;justify-content:center"><a href="${confirmUrl}" style="display:inline-block;background:linear-gradient(135deg,#16a34a 0%,#15803d 100%);color:#fff;font-family:Arial,sans-serif;font-size:15px;font-weight:600;padding:14px 32px;border-radius:14px;text-decoration:none;box-shadow:0 4px 16px rgba(22,163,74,0.25)">Confirm</a><a href="${cancelUrl}" style="display:inline-block;background:linear-gradient(135deg,#dc2626 0%,#991b1b 100%);color:#fff;font-family:Arial,sans-serif;font-size:15px;font-weight:600;padding:14px 32px;border-radius:14px;text-decoration:none;box-shadow:0 4px 16px rgba(220,38,38,0.25)">Decline</a></div></td></tr><tr><td style="padding:24px 40px;text-align:center;background:linear-gradient(135deg,rgba(255,248,240,0.4),rgba(245,166,35,0.04));border-top:1px solid rgba(232,98,26,0.08)"><img src="${logoIcon}" alt="" width="28" style="height:auto;display:block;margin:0 auto 8px;opacity:0.6"/><p style="margin:0 0 4px;font-size:11px;color:#8b7c73;letter-spacing:0.3px">SP-28, Wajidpur, Sector-130, Noida — 201301</p><p style="margin:0;font-size:10px;color:#a8a29e">© 2026 Ayurshala Panchakarma Center</p></td></tr></table></td></tr></table></body></html>`
}
