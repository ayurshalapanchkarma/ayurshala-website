...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!booking)
      return NextResponse.redirect(new URL('/status/cancel?type=not_found', req.url))

    if (booking.status === 'CANCELLED')
      return NextResponse.redirect(new URL(`/status/cancel?type=already_cancelled&booking_id=${booking_id}`, req.url))
