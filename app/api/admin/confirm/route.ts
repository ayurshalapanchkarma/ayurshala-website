...
export async function GET(req: NextRequest) {
  ...
  if (booking.status === 'CONFIRMED') {
    const { data: patient } = await supabase.from('patients').select('full_name,patient_id').eq('id', booking.patient_uuid).single()
    const formattedDate = new Date(booking.preferred_date).toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
    return new NextResponse(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Already Confirmed</title></head><body style="margin:0;padding:0;background:#f0fdf4;font-family:Arial,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center"><div style="max-width:480px;width:90%;margin:40px auto;background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 8px 40px rgba(22,163,74,0.12);border:1px solid rgba(22,163,74,0.2)"><div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);padding:32px 40px;text-align:center;border-bottom:1px solid rgba(22,163,74,0.15)"><div style="width:64px;height:64px;background:rgba(22,163,74,0.15);border-radius:50%;display:inline-flex;align-items:center;justify-content:center;margin-bottom:16px;font-size:28px">✓</div><h1 style="margin:0;font-family:Georgia,serif;font-size:22px;color:#15803d">Booking Already Confirmed</h1><p style="margin:8px 0 0;font-size:13px;color:#6b7280">This appointment was previously confirmed</p></div><div style="padding:28px 40px"><table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;border-radius:14px;border:1px solid #e5e7eb"><tr><td style="padding:12px 20px;border-bottom:1px solid #f3f4f6"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#9ca3af">Patient</span><br><span style="font-size:15px;color:#111827;font-weight:600">${patient?.full_name}</span></td></tr><tr><td style="padding:12px 20px;border-bottom:1px solid #f3f4f6"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#9ca3af">Booking ID</span><br><span style="font-size:15px;color:#E8621A">${booking.booking_id}</span></td></tr><tr><td style="padding:12px 20px;border-bottom:1px solid #f3f4f6"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#9ca3af">Date &amp; Time</span><br><span style="font-size:15px;color:#111827">${formattedDate} · ${booking.preferred_time}</span></td></tr><tr><td style="padding:12px 20px"><span style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#9ca3af">Status</span><br><span style="font-size:14px;color:#16a34a;font-weight:600;background:#dcfce7;padding:4px 12px;border-radius:20px;display:inline-block;margin-top:4px">✓ Confirmed</span></td></tr></table></div></div></body></html>`, { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } })
  }
  ...
  const amountLabel = `₹${booking.booking_type === 'consultation' ? 500 : 1000} — Cash on Arrival`
