...
export async function GET(req: NextRequest) {
  ...
  if (secret !== (process.env.ADMIN_CONFIRM_SECRET ?? 'ayurshala-confirm'))
    return NextResponse.redirect(new URL('/status/confirm?type=unauthorized', req.url))

  if (!booking_id)
    return NextResponse.redirect(new URL('/status/confirm?type=missing', req.url))

  const { data: booking } = await supabase.from('bookings_new').select('*').eq('booking_id', booking_id).single()
  if (!booking)
    return NextResponse.redirect(new URL('/status/confirm?type=not_found', req.url))
  
  if (booking.status === 'CANCELLED')
    return NextResponse.redirect(new URL(`/status/confirm?type=cancelled&booking_id=${booking_id}`, req.url))
  
  if (booking.status === 'CONFIRMED')
    return NextResponse.redirect(new URL(`/status/confirm?type=already_confirmed&booking_id=${booking_id}`, req.url))
  ...
  if (patient?.email) {
    ...
    resend.emails.send({ from, to: patient.email, subject: `Booking Confirmed — ${booking_id}`, html: '<p>Booking confirmed</p>' }).catch(() => {})
  }

  return NextResponse.redirect(new URL(`/status/confirm?type=success&booking_id=${booking_id}&patient_name=${patient?.full_name || 'Patient'}`, req.url))
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (booking.status === 'CANCELLED') {
      return NextResponse.json({ error: 'Cannot confirm cancelled booking' }, { status: 409 })
    }
    if (booking.status === 'CONFIRMED') {
