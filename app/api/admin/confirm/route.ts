...
export async function GET(req: NextRequest) {
  ...
  if (booking.status === 'CONFIRMED') return new NextResponse(`<html><body style="font-family:Arial;text-align:center;padding:60px"><h2 style="color:#16a34a">✓ Already Confirmed</h2><p>${booking_id} was already confirmed.</p></body></html>`, { status: 200, headers: { 'Content-Type': 'text/html' } })
  ...
  return new NextResponse(`<html><body style="font-family:Arial;text-align:center;padding:60px;background:#f0fdf4"><h2 style="color:#16a34a">✓ Booking Confirmed!</h2><p style="color:#555">${booking_id} — ${patient?.full_name}</p><p style="color:#555">Confirmation email sent to patient.</p></body></html>`, { status: 200, headers: { 'Content-Type': 'text/html' } })
}
