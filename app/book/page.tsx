...
type Booking = { id: string; treatment: string; date: string; time: string; paymentmethod: string; status: string; created_at: string }
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      if (u) {
        ...
        // Load past bookings by email
        supabase.from('bookings').select('*').eq('email', u.email!).order('created_at', { ascending: false }).limit(5)
    ...
    <div className="min-h-screen py-24 px-6 relative overflow-hidden">
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        >
          ...
          {/* Sign in prompt / past bookings */}
          {mounted && (user ? (
            pastBookings.length > 0 && (
              <div className={`rounded-2xl p-4 mb-6 border ${dark ? 'border-white/10 bg-white/5' : 'border-brand/15 bg-brand/5'}`}>
                <p className="font-sans text-xs text-stone-400 uppercase tracking-wider mb-3">Your Past Bookings</p>
                <div className="space-y-2">
                  {pastBookings.map(b => (
                    <div key={b.id} className={`flex items-center justify-between text-xs font-sans px-3 py-2 rounded-xl ${dark ? 'bg-white/5' : 'bg-white/60'}`}>
                      <span className="text-stone-600 font-medium">{b.treatment}</span>
                      <span className="text-stone-400">{b.date || '—'} {b.time || ''}</span>
                      <span className={`px-2 py-0.5 rounded-full text-xs ${b.status === 'confirmed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{b.status}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className={`rounded-2xl p-4 mb-6 border flex items-center justify-between gap-4 ${dark ? 'border-white/10 bg-white/5' : 'border-brand/15 bg-brand/5'}`}>
              <p className="font-sans text-sm text-stone-500">Sign in to auto-fill your details &amp; view past bookings</p>
              <button type="button" onClick={() => supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/auth/callback?next=/book` } })}
                className="btn-glass text-xs py-2 px-4 flex items-center gap-1.5 flex-shrink-0">
