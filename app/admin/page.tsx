...
import { Home, LogOut, Moon, Sun, Calendar, Clock, Wallet, TrendingUp, FileText, Package, Users, BarChart3, Settings, Stethoscope, CreditCard, Search, Bell, ChevronRight } from 'lucide-react'
...
const modules = [
  { label: 'Appointments', icon: Calendar, href: '/admin/appointments', color: 'blue', description: 'Manage patient appointments' },
  ...
  { label: 'Reports', icon: BarChart3, href: '/admin/reports', color: 'cyan', description: 'Analytics and reports' },
  { label: 'Settings', icon: Settings, href: '/admin/settings', color: 'slate', description: 'System configuration' },
]

export default function AdminPage() {
  const [mounted, setMounted] = useState(false)
  const [stats, setStats] = useState({ today: 0, pending: 0, revenue: 0, patients: 0, doctors: 0, alerts: 0 })
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      const now = new Date()
      setCurrentTime(now.toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' }))
    }, 1000)
  ...
  const colorMap = {
    blue: { bg: dark ? 'bg-blue-950/30' : 'bg-blue-50', icon: 'text-blue-600 dark:text-blue-400', border: 'border-blue-200 dark:border-blue-900/50' },
    green: { bg: dark ? 'bg-green-950/30' : 'bg-green-50', icon: 'text-green-600 dark:text-green-400', border: 'border-green-200 dark:border-green-900/50' },
    purple: { bg: dark ? 'bg-purple-950/30' : 'bg-purple-50', icon: 'text-purple-600 dark:text-purple-400', border: 'border-purple-200 dark:border-purple-900/50' },
    amber: { bg: dark ? 'bg-amber-950/30' : 'bg-amber-50', icon: 'text-amber-600 dark:text-amber-400', border: 'border-amber-200 dark:border-amber-900/50' },
    indigo: { bg: dark ? 'bg-indigo-950/30' : 'bg-indigo-50', icon: 'text-indigo-600 dark:text-indigo-400', border: 'border-indigo-200 dark:border-indigo-900/50' },
    emerald: { bg: dark ? 'bg-emerald-950/30' : 'bg-emerald-50', icon: 'text-emerald-600 dark:text-emerald-400', border: 'border-emerald-200 dark:border-emerald-900/50' },
    cyan: { bg: dark ? 'bg-cyan-950/30' : 'bg-cyan-50', icon: 'text-cyan-600 dark:text-cyan-400', border: 'border-cyan-200 dark:border-cyan-900/50' },
    slate: { bg: dark ? 'bg-slate-800' : 'bg-slate-100', icon: 'text-slate-600 dark:text-slate-400', border: 'border-slate-200 dark:border-slate-700' },
  }
  ...
  return (
    <AdminGuard>
      <div className={`min-h-screen ${dark ? 'bg-slate-950' : 'bg-white'}`}>
        {/* Header */}
        <header className={`sticky top-0 z-40 border-b ${dark ? 'bg-slate-900/95 border-slate-800' : 'bg-white/95 border-gray-200'} backdrop-blur`}>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 mb-8">
            {[
              ...
              { label: 'Revenue', value: `₹${stats.revenue}`, icon: Wallet },
              { label: 'Patients', value: stats.patients, icon: Users },
              { label: 'Doctors', value: stats.doctors, icon: Stethoscope },
              { label: 'Alerts', value: stats.alerts, icon: Bell },
            ].map((kpi, i) => {
              const Icon = kpi.icon
              return (
                <div key={i} className={`rounded-lg p-4 border transition ${dark ? 'bg-slate-800/50 border-slate-700 hover:bg-slate-700/50' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
              ...
              {modules.map((module, i) => {
                ...
                return (
                  <Link key={i} href={module.href}>
                    <motion.div whileHover={{ y: -4 }} className={`rounded-lg p-4 border cursor-pointer transition ${dark ? 'bg-slate-800/50 border-slate-700 hover:border-slate-600' : 'bg-white border-gray-200 hover:border-orange-300'}`}>
                      <div className={`w-10 h-10 rounded-lg ${colors.bg} flex items-center justify-center mb-3 ${colors.border} border`}>
